...
const NavHeader = () => {
  ...
  return (
    <div>
      <nav onClick={handleOutsideClick} className={`${menuMobile ? 'flex' : 'hidden'} flex-col-reverse items-end fixed top-0 left-0 h-screen w-full p-4 bg-menu-mobile lg:flex lg:flex-row lg:static lg:h-auto lg:w-auto lg:bg-white lg:p-0`}>
        <ul className='flex flex-col px-5 py-2 bg-white h-screen w-full rounded-sm lg:h-auto lg:flex-row lg:p-0 lg:rounded-none lg:w-auto'>
          <li>
        ...
        <button onClick={() => setMenuMobile(null)} className='text-white text-[28px] mb-4 lg:hidden'>
          <IoClose />
        </button>
      </nav>

      <button onClick={() => setMenuMobile(true)} className={`${menuMobile ? 'hidden' : 'block'} text-dark-blue text-2xl lg:hidden`}>
        <AiOutlineMenu />
      </button>
    </div>
  )
